...
df2 <- paths |> map(\(path) read_csv(path, col_names = FALSE)) |> list_rbind() # \(x) is called an anonymous function
colnames(df2) <- c("filename","no.intersections","radius.um","x")
...
### Now write a loop to identify the genes that are up-regulated > 3x fold change and DE in either both groups, only control, or only exp
